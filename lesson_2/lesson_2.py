...
min_page = int(input('write from what page you wanna see result: '))-1
params['page'] = str(min_page)
max_page = int(input('write the last page you wanna see results: '))-1  
page_counter = min_page
session = requests.Session()
while True:
    resp = session.get(url, headers=headers, params=params)
    dom = BeautifulSoup(resp.content, 'html.parser')
    page = dom.find('div', attrs= {'class':'pager'}).find('a', attrs={'data-qa':'pager-next'})
    if page == None or page_counter>max_page:
        break
